...
def load_data(dataset, maxbudget, nth=None, n_top_kw=20, usecost=False, maxcost=1):
    if dataset == 'runtime':
        ...
        F_ = [set(np.random.choice(len(V), np.random.randint(0,maxsz), replace=False)) for _ in range(nth)]
        F = [lambda S, subset=sgs: int(np.any([v in subset for v in S])) for sgs in F_]
    ...
    with open(f'datasets/{dataset}.pkl', 'rb') as fin:
        ...
        if dataset == 'news':
            ...
            union = lambda S: itertools.chain(*[Vset[i] for i in S])
            F = [lambda S, subset=kws: len(subset.intersection(union(S))) / len(subset) for kws in F_]

            if usecost:
                B = np.random.randint(1, np.mean(doc_lens) * maxbudget, size=len(F))
            else:
                B = np.random.randint(1, maxbudget + 1, size=len(F))
        ...
        if dataset.endswith('nist'):
            ...
            R = [max([max(Ds[i][u]) for u in V] + [1e-5]) for i,rc in enumerate(dims)] # avoid 0
            ...
            radius_ = lambda S, rcidx: np.mean([min([Ds[rcidx][min(v,s)][np.abs(v-s)] for s in S]) for v in V])
            ...
            d = lambda u,v,rc: norm(u[rc] - v[rc])
            pred = lambda S, rc: np.array([ y_train[S[np.argmin([d(X_train[s],x,rc) \
                                 for s in S])]] \
                                 for x in X_test])

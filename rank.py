...
def measure(R, V, F, B, c=None):
    ...
    if c is None:
        ...
        b2i = lambda b: len(accb) if b>accb[-1] else np.where(b <= accb)[0][0]
...
def DP(V, F, B, c, eps):
    ...
    # Define DP objective
    largeF = lambda j, cost: [k for k,b in enumerate(B) if cost+c[j] <= b and 2*c[j] > b]
    P = max([F[k]([V[j]]) for j in range(len(V)) for k in largeF(j,0)] + [-1])
    if P == -1 or P == 0:
        # z=0 for all ranking, and return arbitrary ranking
        return idxV
    scale_round = lambda x: math.floor(x / P * len(F) /eps)
    z = lambda j, cost: sum([scale_round(F[k]([V[j]])) for k in largeF(j,cost)])
    ...
    for j in range(1, len(V)):
        ...
        for i in reversed(range(1, nrow)):
            cs = [ cT[(i,j-1)], cT[(i,j)], cT[(i+1,j)] if i<nrow-1 else MAXc ]
...
def greedy(V, F, B, c=None, weight=None):
    ...
    while sum([c[j_] for j_ in ridx]) < bmax:
        ...
        for j, v in enumerate(V):
            ...
            S = ranking + [v]
            cS = cr + c[j]
            score = sum([w * (f(S)-fr) for w,f,b,fr in zip(weight, F, B, frs) if b >= cS])
            score = score / c[j]
            scores[j] = score
            if score > s_max:
                s_max = score
...
def greedy_sr(V, F, B, c=None):
    ...
    '''
    bmax = max(B)
    c = np.ones(len(V), dtype='int') if c is None else c # uniform cost if None
    ranking, ridx, sidx = [], [], set()
    ...
    while sum([c[j_] for j_ in ridx]) < bmax:
        ...
        for j,v in enumerate(V):
            ...
            if ub < s_max:
                scores[j] = ub # important, (1-fr) may be too small and be discarded
                continue
...
def rank_by_quality(V, F):
    score = lambda v: sum([f([v]) for f in F])
